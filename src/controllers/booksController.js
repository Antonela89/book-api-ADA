...
function formatBookData(book) {
	...
	return {
		...
		author: author
			? author.name
			: `Autor Desconocido (ID: ${book.authorId})`,
		publisher: publisher
			? publisher.name
			: `Editorial Desconocida (ID: ${book.publisherId})`,
		year: book.year,
		genre: book.genre,
	};
}
...
const BooksController = {
	...
	getBooksByTitle(title) {
		try {
			const books =
				BooksModel.findBooksByTitle(title).map(formatBookData);
			if (books.length > 0) {
				return ResponseFormatter.formatSuccess(
					`Se encontraron ${books.length} libros con el título "${title}".`,
					books,
				);
			} else {
				return ResponseFormatter.formatError(
	...
	addBook(newBookData) {
		try {
			...
			const rawAuthorName = getCaseInsensitiveValue(
				newBookData,
				'authorName',
			);
			const rawPublisherName = getCaseInsensitiveValue(
				newBookData,
				'publisherName',
			);
			...
			if (
				...
				return ResponseFormatter.formatError(
					'Faltan datos obligatorios (title, authorName, publisherName, year, genre).',
				);
			}
			...
			const isDuplicate = books.some(
				(b) => b.title.toLowerCase() === rawTitle.toLowerCase(),
			);
			if (isDuplicate) {
				return ResponseFormatter.formatError(
					`Ya existe un libro con el título exacto "${rawTitle}".`,
				);
			}
			...
			if (authors.length === 0)
				return ResponseFormatter.formatError(
					`El autor "${rawAuthorName}" no existe.`,
				);
			if (authors.length > 1)
				return ResponseFormatter.formatError(
					`El nombre de autor "${rawAuthorName}" es ambiguo.`,
				);

			// Valida el nombre de la editorial
			const publishers =
				PublishersModel.findPublishersByName(rawPublisherName);
			if (publishers.length === 0)
				return ResponseFormatter.formatError(
					`La editorial "${rawPublisherName}" no existe.`,
				);
			if (publishers.length > 1)
				return ResponseFormatter.formatError(
					`El nombre de editorial "${rawPublisherName}" es ambiguo.`,
				);

			// Creación del objeto a guardar
			const bookToSave = {
				title: toCapitalCase(rawTitle.trim()),
				authorId: authors[0].id,
				publisherId: publishers[0].id,
				year: parseInt(rawYear),
				genre: toCapitalCase(rawGenre.trim()),
			};
			...
			return ResponseFormatter.formatError(
				'Ocurrió un error al añadir el libro.',
			);
		}
	},
	...
	updateBook(id, updatedBookData) {
		try {
			...
			if (
				getCaseInsensitiveValue(updatedBookData, 'authorName') ||
				getCaseInsensitiveValue(updatedBookData, 'publisherName')
			) {
				return ResponseFormatter.formatError(
					'Para cambiar autor o editorial, debe eliminar y volver a crear el libro.',
				);
			}
			...
			const dataToUpdate = buildUpdateObject(
				updatedBookData,
				allowedFields,
			);
			...
			if (Object.keys(dataToUpdate).length === 0) {
				...
				return ResponseFormatter.formatError(
					`No se proporcionaron datos válidos (${validFields}) para actualizar.`,
				);
			}
			...
			if (success) {
				return ResponseFormatter.formatSuccess(
					`Libro con ID ${id} actualizado correctamente.`,
				);
			} else {
				return ResponseFormatter.formatError(
					`No se encontró ningún libro con el ID ${id} para actualizar.`,
				);
			}
		} catch (error) {
	...
	deleteBook(id) {
		try {
			...
			if (success) {
				...
				return ResponseFormatter.formatError(
					`No se encontró ningún libro con el ID ${id} para eliminar.`,
				);
			}
		} catch (error) {
